...
/* 3.2  Ubicaciones */
INSERT INTO CALLE_DE_LUNA.BI_Dimension_Ubicacion (provincia_codigo, localidad_codigo, provincia, localidad)
SELECT DISTINCT 
    p.provincia_id, 
    l.loc_id, 
    p.provincia_nombre, 
    l.loc_nombre
FROM CALLE_DE_LUNA.Provincia p
JOIN CALLE_DE_LUNA.Localidad l ON l.loc_provincia = p.provincia_id
WHERE NOT EXISTS (
    SELECT 1 
    FROM CALLE_DE_LUNA.BI_Dimension_Ubicacion ubi
    WHERE ubi.provincia = p.provincia_nombre
      AND ubi.localidad = l.loc_nombre
);
...
/*SELECT DISTINCT sil_codigo, sil_modelo, sil_medida
FROM CALLE_DE_LUNA.Sillon
WHERE NOT EXISTS (
      SELECT 1 FROM CALLE_DE_LUNA.BI_Dimension_Modelo
      WHERE codigo_externo = sil_id
        AND modelo_codigo     = sil_modelo
        AND medida         = sil_medida);*/
...
-- 4. HECHOS
---------------------------------------------------------------
create function CALLE_DE_LUNA.sonIguales(@anio int , @mes int ,@cuatri int , @fecha datetime2(6))
returns int 
as
begin 
	RETURN CASE 
	WHEN YEAR(@fecha) = @anio 
        AND  ceiling(datepart(month, @fecha)/4.0) = @cuatri
        AND MONTH(@fecha) = @mes
        THEN 1
        ELSE 0
	END
...
/*SELECT 
    ...
    dubic.ubicacion_id,
   SUM(CASE WHEN tm.tip_id = dmat.tipo_material_id AND COALESCE(ic.item_sub_total, 0) != 0 THEN 1 ELSE 0 END) AS cantidad_compras,
	SUM(CASE WHEN tm.tip_id = dmat.tipo_material_id THEN COALESCE(ic.item_sub_total, 0) ELSE 0 END) AS total_compras
FROM CALLE_DE_LUNA.Item_Compra ic
	right JOIN CALLE_DE_LUNA.Compra c ON c.compra_id = ic.compra_id
	...
	group by dsuc.sucursal_id, dtiempo.tiempo_id,dmat.tipo_material_id, dubic.ubicacion_id */
SELECT 
    dtiempo.tiempo_id,
    dsuc.sucursal_id,
    dmat.tipo_material_id,
    ...
    -- Cantidad de compras (ítems con subtotal distinto de 0

    -- Total acumulado de subtotales
    SUM(
        CASE  
            WHEN tm.tip_id = dmat_map.tipo_material_id 
            THEN COALESCE(ic.item_sub_total, 0) 
            ELSE 0 
        END
    ) AS total_compras

FROM CALLE_DE_LUNA.BI_Dimension_Tiempo dtiempo
CROSS JOIN CALLE_DE_LUNA.BI_Dimension_Sucursal dsuc
CROSS JOIN CALLE_DE_LUNA.BI_Dimension_Tipo_Material dmat

JOIN CALLE_DE_LUNA.Sucursal suc ON suc.sucu_nroSucursal = dsuc.nro_sucursal
JOIN CALLE_DE_LUNA.Direccion dir ON dir.dir_id = suc.sucu_direccion
JOIN CALLE_DE_LUNA.Localidad loc ON loc.loc_id = dir.dir_localidad
JOIN CALLE_DE_LUNA.Provincia prov ON prov.provincia_id = loc.loc_provincia
JOIN CALLE_DE_LUNA.BI_Dimension_Ubicacion dubic 
     ON dubic.localidad_codigo = loc.loc_id 
     AND dubic.provincia_codigo = prov.provincia_id

LEFT JOIN CALLE_DE_LUNA.Compra c 
     ON c.comp_sucursal = dsuc.nro_sucursal 
     AND CALLE_DE_LUNA.sonIguales(dtiempo.anio, dtiempo.mes, dtiempo.cuatrimestre, c.comp_fecha) = 1

LEFT JOIN CALLE_DE_LUNA.Item_Compra ic ON ic.compra_id = c.compra_id 

LEFT JOIN CALLE_DE_LUNA.Tipo_Material tm ON tm.tip_id = ic.item_material

LEFT JOIN CALLE_DE_LUNA.BI_Dimension_Tipo_Material dmat_map
     ON dmat_map.tipo_descripcion = 
        CASE 
            WHEN LEFT(tm.tip_descripcion,1) = 'T' THEN 'Tela'
            WHEN LEFT(tm.tip_descripcion,1) = 'M' THEN 'Madera'
            ELSE 'Relleno'
        END
     AND dmat_map.tipo_material_id = dmat.tipo_material_id
...
/*----------------------------------------------------------------
  4.2  VENTAS
----------------------------------------------------------------*/
INSERT INTO CALLE_DE_LUNA.BI_Hecho_Ventas(tiempo,sucursal,turno,modelo_sillon,ubicacion,cliente,rango_etario,estado_pedido, cantidad, tiempo_fabricacion, subtotal)
SELECT
    dtiempo.tiempo_id,
    dsuc.sucursal_id,
    dturno.turno_id,
...
SELECT dtiempo.anio, dtiempo.cuatrimestre, ubi.provincia, SUM(hv.subtotal)/ SUM(hv.cantidad) AS factura_prom_mensual
FROM CALLE_DE_LUNA.BI_Hecho_Ventas hv
	JOIN CALLE_DE_LUNA.BI_Dimension_Tiempo dtiempo ON hv.tiempo = dtiempo.tiempo_id
	JOIN CALLE_DE_LUNA.BI_Dimension_Ubicacion ubi ON hv.ubicacion= ubi.ubicacion_id
GROUP BY dtiempo.anio, dtiempo.cuatrimestre, ubi.provincia;
GO
...
WITH Rank AS (
    SELECT 
        ...
        dsil.modelo_codigo,
        dsil.modelo_descripcion, -- suponiendo que exista
...
/* 5.4 Volumen de pedidos por turno */ --BIEN
...
/* 5.6 Tiempo promedio de fabricación */ --BIEN
...
/* 5.8 Compras por tipo material */--BIEN
...
/* 5.10 Top-3 localidades costo envío */--BIEN
